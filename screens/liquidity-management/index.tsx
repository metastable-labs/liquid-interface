...
const LiquidityManagement = ({ id, type }: ILiquidityManagement) => {
  ...
  const invalidAmount =
    (tab === 'stake' && parseFloat(removeCommasFromNumber(amount)) > lpBalance) ||
    (tab === 'unstake' && parseFloat(removeCommasFromNumber(amount)) > stakedBalance);
      ...
      <View style={styles.topContainer}>
        <View style={styles.tabs}>
          {tabs.map((text) => (
            <TouchableOpacity
              key={text}
              style={[styles.tab, text === tab ? styles.activeTab : styles.inactiveTab]}
              onPress={() => {
                setTab(text as ILiquidityManagement['type']);
                setAmount('0');
              }}
            >
              <Text style={[styles.tabText, text === tab ? styles.activeTabText : styles.inactiveTabText]}>{text}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.main}>
          <View style={[styles.inputAndBalance, invalidAmount && styles.invalidContainer]}>
            <TextInput
              value={amount}
            ...
            <View style={styles.balanceContainer}>
              ...
              <View style={styles.balanceInfo}>
                <Text style={[styles.balanceTitle, invalidAmount && styles.invalidText]}>{tab === 'stake' ? 'LP' : 'Staked'} BAL:</Text>
                <Text style={[styles.balanceValue, invalidAmount && styles.invalidText]}>{balance.toLocaleString()} LP</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.partitions}>
            {balancePartitions.map(({ text, action }, index) => (
              <TouchableOpacity key={index} style={styles.partition} onPress={action}>
